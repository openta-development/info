...
Plus one of: - ``GIT_ALLOW_PUSH: true`` (for backing up) -
``GIT_ALLOW_PULL: true`` (for restoring)

**Note**: You can’t have both enabled on the same course (safety
feature).

These are inserted as optional fields int Course -> Options at the end.
Press “Add Field” and insert these. If you are pulling from a public
repository, you only need ``GIT_ALLOW_PULL  : true`` and
``GIT_REPO : XXXX``
...
2. Select the entire course, and read the warning
3. Wait 1-2 minutes
4. Done!
